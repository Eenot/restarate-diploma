...
public class ReviewDbStorageImpl implements ReviewStorage {
    ...
    public Review create(Review review) {
        String sql = "insert into Reviews(CONTENT, POSITIVE, USER_ID, DISH_ID) " +
                "values (?, ?, ?, ?)";
        ...
        jdbcTemplate.update(connection -> {
                    ...
                    stmt.setLong(4, review.getDishId());
                    return stmt;
                },
                keyHolder);
    ...
    @Override
    public List<Review> getReviewList(Long dishId, Long count) {
        String sql = "SELECT * FROM REVIEWS \n";
        if (dishId != null) {
            sql += "WHERE dish_id = ? \n" +
                    "ORDER BY USEFUL desc \n" +
                    "LIMIT ?";
            return jdbcTemplate.query(sql, (rs, rowNum) -> makeReview(rs), dishId, count);
        }
